...
Class FORMS
{
	...
	##########################################################################################################################
	 public static function forms_SelectRowByLatinName($latin_name)
    {
        $sql = 'CALL `sp_forms_SelectRowByLatinName`(:latin_name)';
        $params = array (':latin_name' => $latin_name);
        DatabaseHandler::Close(); 
        return DatabaseHandler::GetRow($sql, $params);
    }
	public static function forms_UpdateActivate($id, $activate)
    {
        $sql = 'CALL `sp_forms_UpdateActivate`(:id, :activate)';
        $params = array (':id' => $id, ':activate' => $activate);
        DatabaseHandler::Close(); 
        return DatabaseHandler::Execute($sql, $params);
    }
	
	  public static function forms_SelectAllWhereActivate()
    {
        $sql = 'CALL `sp_forms_SelectAllWhereActivate`()';
        DatabaseHandler::Close(); 
        return DatabaseHandler::GetAll($sql);
    }
}
