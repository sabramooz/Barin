			...
			$this->form = FORMS::forms_SelectRow($_GET['id']);
					...
					foreach($content_rows as $content_row){
						...
						if($content_row['text']==1){
							$htmlForms .= "<label for='$content_row[name]'>$content_row[label]</label><input type='text' disabled name='$content_row[name]' class='form-control' required ><br>\n";
							}
					...
					$this->formsAnswersTitle .= "<th class='text-center'> ردیف </th>";
					$this->formsAnswersTitle .= "<th class='text-center'> شخص </th>";
					foreach($formContent as $title){
						$this->formsAnswersTitle .= "<th class='text-center'>".$title['label']."</th>";
